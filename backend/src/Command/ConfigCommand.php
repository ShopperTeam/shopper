...
class ConfigCommand extends Command
{
    ...
    public function __construct()
    {
        ...
        $this->defaultDB = new class
        {
            public $userName = 'root';
            public $passWord = '';
            public $dbname = 'shopper';
            public $port = '3306';
    ...
    protected function checkEnvLocal(InputInterface $input, OutputInterface $output): bool
    {
        ...
        if (!$fileExist || $forceOption) {
            ...
            $provider = $helper->ask($input, $output, new Question("Fournisseur [". $this->defaultDB->provider ."] : ", $this->defaultDB->provider));
            $userName = $helper->ask($input, $output, new Question("Nom d'utilisateur [". $this->defaultDB->userName ."] : ", $this->defaultDB->userName));
            $passWord = $helper->ask($input, $output, new Question("Mot de passe [". $this->defaultDB->passWord ."] : ", $this->defaultDB->passWord));
            $dbname = $helper->ask($input, $output, new Question("Non de la base de donnée [". $this->defaultDB->dbname ."] : ", $this->defaultDB->dbname));
            $port = $helper->ask($input, $output, new Question("Numéro de port [". $this->defaultDB->port ."] : ", $this->defaultDB->port));
            $host = $helper->ask($input, $output, new Question("Host [". $this->defaultDB->host ."] : ", $this->defaultDB->host));

            $dataBaseUrl = "$provider://$userName:$passWord@$host:$port/$dbname\n";
